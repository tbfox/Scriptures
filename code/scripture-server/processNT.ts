...
// Load and process New Testament verses
console.log("📖 Loading New Testament data...");
const file = Bun.file(
    "/Users/tristanbarrow/Scriptures/code/scripture-server/res/nt.json",
);
...
const insert_verses = verses.map(({ text, reference }): LongVerse => {
    // Parse reference like "Matthew 1:1" into parts
...
console.log("🔄 Processing New Testament verses...");
...
console.log(
    `✅ Successfully inserted ${processed} New Testament verses into database`,
);
...
const sampleVerses = db
    .query("SELECT * FROM verses WHERE source = 'nt' LIMIT 5")
    .all();

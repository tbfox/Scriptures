...
// Load and process Doctrine and Covenants verses
console.log("📖 Loading Doctrine and Covenants data...");
const file = Bun.file(
    "/Users/tristanbarrow/Scriptures/code/scripture-server/res/dc-testament.json",
);
...
const insert_verses = verses.map(({ text, reference }): LongVerse => {
    // Parse reference like "D&C 1:1" into parts
...
console.log("🔄 Processing Doctrine and Covenants verses...");
...
console.log(
    `✅ Successfully inserted ${processed} Doctrine and Covenants verses into database`,
);
...
const dcCount = db
    .query("SELECT COUNT(*) as count FROM verses WHERE source = 'dc'")
    .get() as { count: number };
console.log(`📜 Doctrine and Covenants verses: ${dcCount.count}`);
...
// Show sample D&C data
console.log("\n📋 Sample Doctrine and Covenants verses:");
const sampleVerses = db
    .query("SELECT * FROM verses WHERE source = 'dc' LIMIT 5")
    .all();
